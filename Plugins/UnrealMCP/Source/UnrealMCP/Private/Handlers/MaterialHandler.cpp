...
TSharedPtr<FJsonObject> FMaterialHandler::AddMaterialExpression(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!IsValid(Material))
    {
        ...
        FARFilter Filter;
        // UE5.5: FARFilter에 AssetNames 필드 없음 → ClassPaths+PackagePaths로 조회 후 이름 필터
        ...
        FAssetData* Found = Assets.FindByPredicate([&](const FAssetData& AD){ return AD.AssetName == AssetFName; });
        if (Found)
        {
            Material = Cast<UMaterial>(Found->GetAsset());
        }
    }
